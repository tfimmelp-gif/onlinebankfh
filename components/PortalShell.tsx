...
async function normalizeBrandLogoFile(file:File) {
  ...
  try {
    ...
    context.drawImage(bitmap,0,0);const image=context.getImageData(0,0,canvas.width,canvas.height);const pixels=image.data;
    const cornerIndexes=[0,(canvas.width-1)*4,(canvas.height-1)*canvas.width*4,(canvas.height*canvas.width-1)*4];
    const transparentCorners=cornerIndexes.every(index=>pixels[index+3]<24);
    const cornerColors=cornerIndexes.map(index=>[pixels[index],pixels[index+1],pixels[index+2]] as const);
    const lightBackground=cornerColors.every(([red,green,blue])=>Math.min(red,green,blue)>232&&Math.max(red,green,blue)-Math.min(red,green,blue)<18);
    if(!transparentCorners&&!lightBackground)return null;
    if(lightBackground){
      const background=cornerColors.reduce((total,color)=>[total[0]+color[0],total[1]+color[1],total[2]+color[2]],[0,0,0]).map(value=>value/4);
      for(let index=0;index<pixels.length;index+=4){const distance=Math.max(Math.abs(pixels[index]-background[0]),Math.abs(pixels[index+1]-background[1]),Math.abs(pixels[index+2]-background[2]));const matte=distance<=9?0:distance>=42?1:(distance-9)/33;pixels[index+3]=Math.round(pixels[index+3]*matte);}
      context.putImageData(image,0,0);
    }
    let left=canvas.width,top=canvas.height,right=-1,bottom=-1;
    for(let y=0;y<canvas.height;y+=1)for(let x=0;x<canvas.width;x+=1){if(pixels[(y*canvas.width+x)*4+3]>16){left=Math.min(left,x);top=Math.min(top,y);right=Math.max(right,x);bottom=Math.max(bottom,y);}}
    if(right<left||bottom<top)return null;
    const contentWidth=right-left+1,contentHeight=bottom-top+1;
    const alreadyTight=transparentCorners&&left<canvas.width*.08&&top<canvas.height*.08&&(canvas.width-1-right)<canvas.width*.08&&(canvas.height-1-bottom)<canvas.height*.08;
    if(alreadyTight)return null;
    const padding=Math.max(4,Math.round(Math.max(contentWidth,contentHeight)*.035));
    const cropLeft=Math.max(0,left-padding),cropTop=Math.max(0,top-padding),cropRight=Math.min(canvas.width-1,right+padding),cropBottom=Math.min(canvas.height-1,bottom+padding);
    const output=document.createElement("canvas");output.width=cropRight-cropLeft+1;output.height=cropBottom-cropTop+1;
    output.getContext("2d")?.drawImage(canvas,cropLeft,cropTop,output.width,output.height,0,0,output.width,output.height);
    const blob=await new Promise<Blob>((resolve,reject)=>output.toBlob(value=>value?resolve(value):reject(new Error("BRAND_LOGO_PROCESSING_FAILED")),"image/png"));
    return new File([blob],file.name.replace(/\.[^.]+$/,"")+"-transparent.png",{type:"image/png"});
  } finally {bitmap.close();}
}

async function prepareBrandLogoUpload(file:File){try{return await normalizeBrandLogoFile(file)??file;}catch{return file;}}

function AdminBrandingWorkspace(){
  const {brandProfiles,mutate}=useBankingData();const [brandId,setBrandId]=useState("");const selected=brandProfiles.find(brand=>brand.id===brandId);const [bankName,setBankName]=useState("Northstar Bank");const [shortName,setShortName]=useState("NORTHSTAR");const [supportEmail,setSupportEmail]=useState("support@northstar.test");const [logoUrl,setLogoUrl]=useState("");const [primaryColor,setPrimaryColor]=useState("#2855d9");const [notice,setNotice]=useState("");const [error,setError]=useState("");const [saving,setSaving]=useState(false);const initializedBrandSelection=useRef(false);const optimizedLogoUrls=useRef(new Set<string>());
  ...
  useEffect(()=>{const current=selected?.logoUrl;if(!selected||!current||optimizedLogoUrls.current.has(current))return;optimizedLogoUrls.current.add(current);let cancelled=false;void (async()=>{const sourceResponse=await fetch(current,{cache:"no-store"});if(!sourceResponse.ok)return;const sourceBlob=await sourceResponse.blob();const normalized=await normalizeBrandLogoFile(new File([sourceBlob],"saved-logo",{type:sourceBlob.type||"image/png"}));if(!normalized||cancelled)return;const form=new FormData();form.set("file",normalized);const uploadResponse=await fetch("/admin/api/brand-logo",{method:"POST",body:form});const uploaded=await uploadResponse.json() as {logoUrl?:string};if(!uploadResponse.ok||!uploaded.logoUrl)return;await mutate({action:"BRAND_SAVE",brandId:selected.id,bankName:selected.bankName,shortName:selected.shortName,supportEmail:selected.supportEmail,logoUrl:uploaded.logoUrl,primaryColor:selected.primaryColor});if(cancelled)return;setLogoUrl(uploaded.logoUrl);const channel=new BroadcastChannel("northstar-brand");channel.postMessage({updated:true});channel.close();setNotice(selected.active?"The active logo was cropped, made transparent, and refreshed across the site.":"The saved logo was cropped and made transparent.");})().catch(()=>undefined);return()=>{cancelled=true;};},[selected,mutate]);
...
function AdminCustomersWorkspace() {
  ...
  const [profileForm,setProfileForm]=useState({firstName:"",lastName:"",email:"",dateOfBirth:"",phone:"",idType:"",idNumber:"",addressLine1:"",addressLine2:"",city:"",stateRegion:"",postalCode:"",countryCode:"",occupation:"",reason:""});
  ...
  function manage(userId:string) {
    ...
    setReason("");setTemporaryPassword("");setPasswordReason("");setError("");setProfileError("");
    if(customer)setProfileForm({firstName:customer.firstName,lastName:customer.lastName,email:customer.email,dateOfBirth:customer.dateOfBirth??"",phone:customer.phone??"",idType:customer.idType??"",idNumber:customer.idNumber??"",addressLine1:customer.addressLine1??"",addressLine2:customer.addressLine2??"",city:customer.city??"",stateRegion:customer.stateRegion??"",postalCode:customer.postalCode??"",countryCode:customer.countryCode??"",occupation:customer.occupation??"",reason:""});
  }

  function updateProfileField(field:keyof typeof profileForm,value:string){setProfileError("");setProfileForm((current)=>({...current,[field]:value}));}

  async function savePersonalInformation(event:React.FormEvent){
    event.preventDefault();if(!selected)return;
    const firstName=profileForm.firstName.trim();
    const lastName=profileForm.lastName.trim();
    const email=profileForm.email.trim().toLowerCase();
    const changeReason=profileForm.reason.trim();
    ...
    if(profileForm.phone.trim()&&!/^[+0-9 ().-]{5,32}$/.test(profileForm.phone.trim())){setProfileError("Enter a valid phone number using digits, spaces, parentheses, +, period, or hyphen.");return;}
    if(profileForm.countryCode.trim()&&!/^[A-Za-z]{2}$/.test(profileForm.countryCode.trim())){setProfileError("Country code must contain exactly two letters.");return;}
    if(changeReason.length<3){setProfileError("Enter a change reason of at least 3 characters.");return;}
    setSubmitting(true);setError("");setProfileError("");
    try{
      const result=await mutate({action:"CUSTOMER_PROFILE_UPDATE",userId:selected.userId,...profileForm,firstName,lastName,email,reason:changeReason});
      setNotice(`Personal information for ${firstName} ${lastName} was updated.${result.sessionsRevoked?" Existing sessions were revoked because the email changed.":""}`);
      setProfileForm((current)=>({...current,firstName,lastName,email,reason:""}));
    }catch(updateError){setProfileError(updateError instanceof Error?updateError.message.replaceAll("_"," "):"Customer profile update failed");}
  ...
  return <>
    ...
    {selected&&<div className="customer-admin-drawer">
      ...
      <div className="customer-admin-drawer-grid">
        <form className="customer-control-form customer-personal-information-form" onSubmit={savePersonalInformation} noValidate><div className="section-title"><div><h2>Personal information</h2><p>Edit verified identity, contact, and address details. Every change requires a reason and is retained in the activity trail.</p></div><span className="status-pill">Audited</span></div><div className="form-row"><div className="field"><label>FIRST NAME</label><input value={profileForm.firstName} onChange={event=>updateProfileField("firstName",event.target.value)} maxLength={100}/></div><div className="field"><label>LAST NAME</label><input value={profileForm.lastName} onChange={event=>updateProfileField("lastName",event.target.value)} maxLength={100}/></div></div><div className="form-row"><div className="field"><label>EMAIL ADDRESS</label><input type="email" value={profileForm.email} onChange={event=>updateProfileField("email",event.target.value)}/><small>Changing email signs out existing sessions and requires verification at the next login.</small></div><div className="field"><label>DATE OF BIRTH</label><input type="date" value={profileForm.dateOfBirth} onChange={event=>updateProfileField("dateOfBirth",event.target.value)}/></div></div><div className="form-row"><div className="field"><label>PHONE NUMBER</label><input type="tel" value={profileForm.phone} onChange={event=>updateProfileField("phone",event.target.value)} maxLength={32}/></div><div className="field"><label>OCCUPATION</label><input value={profileForm.occupation} onChange={event=>updateProfileField("occupation",event.target.value)} maxLength={150}/></div></div><div className="form-row"><div className="field"><label>IDENTIFICATION TYPE</label><input value={profileForm.idType} onChange={event=>updateProfileField("idType",event.target.value)} maxLength={50}/></div><div className="field"><label>IDENTIFICATION NUMBER</label><input value={profileForm.idNumber} onChange={event=>updateProfileField("idNumber",event.target.value)} maxLength={100}/></div></div><div className="field"><label>ADDRESS LINE 1</label><input value={profileForm.addressLine1} onChange={event=>updateProfileField("addressLine1",event.target.value)} maxLength={200}/></div><div className="field"><label>ADDRESS LINE 2</label><input value={profileForm.addressLine2} onChange={event=>updateProfileField("addressLine2",event.target.value)} maxLength={200}/></div><div className="form-row"><div className="field"><label>CITY</label><input value={profileForm.city} onChange={event=>updateProfileField("city",event.target.value)} maxLength={100}/></div><div className="field"><label>STATE / REGION</label><input value={profileForm.stateRegion} onChange={event=>updateProfileField("stateRegion",event.target.value)} maxLength={100}/></div></div><div className="form-row"><div className="field"><label>POSTAL CODE</label><input value={profileForm.postalCode} onChange={event=>updateProfileField("postalCode",event.target.value)} maxLength={20}/></div><div className="field"><label>COUNTRY CODE</label><input value={profileForm.countryCode} onChange={event=>updateProfileField("countryCode",event.target.value.toUpperCase().slice(0,2))} placeholder="US" maxLength={2}/></div></div><div className="field"><label>REQUIRED CHANGE REASON</label><textarea value={profileForm.reason} onChange={event=>updateProfileField("reason",event.target.value)} maxLength={1000} placeholder="Explain why this customer information is being changed"/><small>Enter at least 3 characters.</small></div>{profileError&&<div className="auth-error profile-save-error" role="alert">{profileError}</div>}<button type="submit" className="admin-execute" disabled={submitting}>{submitting?"Saving…":"Save personal information"}</button></form>
