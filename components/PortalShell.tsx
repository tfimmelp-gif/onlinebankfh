...
function AdminCustomersWorkspace() {
  ...
  function updateProfileField(field:keyof typeof profileForm,value:string){setProfileForm((current)=>({...current,[field]:value}));}

  async function savePersonalInformation(event:React.FormEvent){
    event.preventDefault();if(!selected)return;
    const changeReason=profileForm.reason.trim();
    if(changeReason.length<3){setError("Enter a change reason of at least 3 characters.");return;}
    setSubmitting(true);setError("");
    try{
      const result=await mutate({action:"CUSTOMER_PROFILE_UPDATE",userId:selected.userId,...profileForm,reason:changeReason});
      setNotice(`Personal information for ${profileForm.firstName} ${profileForm.lastName} was updated.${result.sessionsRevoked?" Existing sessions were revoked because the email changed.":""}`);
      setProfileForm((current)=>({...current,reason:""}));
    }catch(updateError){setError(updateError instanceof Error?updateError.message.replaceAll("_"," "):"Customer profile update failed");}
